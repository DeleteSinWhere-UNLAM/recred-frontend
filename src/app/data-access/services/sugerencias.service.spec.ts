...
describe('SugerenciasService', () => {
  ...
  it('debería contener producto original', () => {
    ...
    expect(data.some((s) => s.productoOriginal.length > 0)).toBeTrue();
  });

  it('debería tener productos sugeridos', () => {
    const data = service.getSugerencias();

    expect(data.some((s) => s.productosSugeridos.length > 0)).toBeTrue();
  });
  ...
  it('debería tener resumen y motivo IA', () => {
    ...
    expect(
      data.every((s) => s.resumen.length > 0 && s.motivoIA.length > 0),
    ).toBeTrue();
  });
  ...
  it('debería tener modelo IA definido', () => {
    ...
    expect(data.every((s) => s.modeloIA.includes('gemini'))).toBeTrue();
  });
});
