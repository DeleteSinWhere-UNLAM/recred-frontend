...
export class CierreDiarioService {
  ...
  getReporteDiario(buffetId: string, date: string): Observable<ReporteDiario> {
  ...
  getReporteDiarioCsvUrl(buffetId: string, date: string): string {
    const encodedDate = encodeURIComponent(date);

    return `${this.kiosquerosUrl}/${encodeURIComponent(buffetId)}/reports/daily.csv?date=${encodedDate}`;
  }

  downloadReporteDiarioCsv(buffetId: string, date: string): Observable<Blob> {
  ...
  refreshAfterClose(buffetId: string, date: string): Observable<ReporteDiario> {
    return forkJoin({
      ...
      alerts: this.http
        ...
        .pipe(catchError(() => of(null))),
      report: this.getReporteDiario(buffetId, date),
