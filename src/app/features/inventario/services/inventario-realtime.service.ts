...
import { EventoInventarioRealtime } from '../models/inventario.model';
...
interface InventarioRealtimeHandlers {
  onOpen?: () => void;
  onClose?: () => void;
...
export class InventarioRealtimeService {
  ...
  private readonly subscribers = new Map<number, InventarioRealtimeHandlers>();
  ...
  connect(
    buffetId: string,
    handlers: InventarioRealtimeHandlers,
  ): AbortController {
  ...
  private forEachSubscriber(
    callback: (handlers: InventarioRealtimeHandlers) => void,
  ): void {
