...
export function isHighReservation(product: ItemInventario): boolean {
  return getReservationRatio(product) >= HIGH_RESERVATION_THRESHOLD;
}

export function getReservationRatio(product: ItemInventario): number {
...
export function getAvailabilityRatio(product: ItemInventario): number {
  const reserved = normalizeStock(product.stockReservado);
  const available = normalizeStock(product.stockDisponible);
  const total = available + reserved;
...
export function compareByOperationalStatus(
  first: ItemInventario,
  second: ItemInventario,
): number {
