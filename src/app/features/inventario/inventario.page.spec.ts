...
describe('InventarioPage', () => {
  ...
  const mockInventory: ItemInventario[] = [
    {
      productId: '1',
      nombre: 'Alfajor',
      precio: 1200,
  ...
  const mockStockMovements: MovimientoStock[] = [
    {
      id: 'movement-old',
  ...
  beforeEach(async () => {
    ...
    productServiceMock.create.and.returnValue(of(createdProducto));
    productServiceMock.getById.and.returnValue(of(mockProductos[0]));
    productServiceMock.update.and.returnValue(of(mockProductos[0]));
  ...
  it('deberia filtrar productos con alta reserva', () => {
    const highReservationProducto: ItemInventario = {
      ...mockInventory[0],
      productId: 'alta-reserva',
      stockDisponible: 3,
      stockReservado: 4,
    };
    const products: ItemInventario[] = [
      mockInventory[0],
      highReservationProducto,
    ];
    component.products = products;

    component.setFilter('ALTA_RESERVA');

    expect(component.filteredProducts).toEqual([highReservationProducto]);
  });

  it('deberia filtrar productos no disponibles', () => {
    const pausedProducto: ItemInventario = {
      ...mockInventory[0],
      productId: 'pausado',
    ...
    component.products = [...mockInventory, pausedProducto];

    component.setFilter('PAUSADO');

    expect(component.filteredProducts).toEqual([pausedProducto]);
  ...
  it('deberia gestionar modo stock exacto reutilizando valores existentes', () => {
    ...
    fixture.detectChanges();
    component.openInventoryManagement(pausedProducto);
  ...
  describe('Gestión del formulario', () => {
    ...
    it('debería asignar selectedProduct y mostrar el formulario al llamar openEditForm', () => {
      component.openEditForm(mockProductos[0]);
    ...
    it('debería eliminar el producto y actualizar la lista al confirmar', () => {
      ...
      expect(productServiceMock.delete).toHaveBeenCalledWith(mockProductos[0].id);
