...
export class InventoryRealtimeService {
  ...
  private readonly subscribers = new Map<number, InventoryRealtimeHandlers>();
      ...
      await fetchEventSource(
        `${environment.apiUrl}/kiosqueros/${buffetId}/events`,
        {
          ...
          onmessage: (event) => {
            ...
            const normalizedEvent: RealtimeInventoryEvent = {
              ...payload,
              type: eventType,
            };
  ...
  private notifyRefresh(event: RealtimeInventoryEvent): void {
    this.forEachSubscriber((handlers) => handlers.onRefresh(event));
  }

  private notifyPurchaseCreated(event: RealtimeInventoryEvent): void {
  ...
  private forEachSubscriber(
    callback: (handlers: InventoryRealtimeHandlers) => void,
  ): void {
